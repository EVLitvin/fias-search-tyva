DROP TABLE IF EXISTS as_object_levels CASCADE;
DROP TABLE IF EXISTS as_addr_obj_types CASCADE;
DROP TABLE IF EXISTS as_param_types CASCADE;
DROP TABLE IF EXISTS as_house_types CASCADE;
...
CREATE TABLE IF NOT EXISTS as_addr_obj_types
...
(
    ...
    LEVEL       INTEGER      NOT NULL REFERENCES as_object_levels (LEVEL) ON DELETE CASCADE, -- Уровень адресного объекта
...
CREATE TABLE IF NOT EXISTS as_param_types
...
CREATE TABLE IF NOT EXISTS as_house_types
...
-- Состав и структура файла со сведениями о реестре GUID объектов. Сведения об адресном элементе в части его идентификаторов
(
    ...
    LEVELID    INTEGER     NOT NULL REFERENCES as_object_levels (LEVEL) ON DELETE CASCADE, --Уровень объекта
...
(
    ...
    OBJECTID   BIGINT       NOT NULL REFERENCES as_reestr_objects (OBJECTID) ON DELETE CASCADE, -- Глобальный уникальный идентификатор объекта типа INTEGER
    ...
    LEVEL      SMALLINT     NOT NULL REFERENCES as_object_levels (LEVEL) ON DELETE CASCADE,     -- Уровень адресного объекта
...
(
    ID          BIGINT       NOT NULL PRIMARY KEY,                                               -- Уникальный идентификатор записи. Ключевое поле
    OBJECTID    BIGINT       NOT NULL REFERENCES as_reestr_objects (OBJECTID) ON DELETE CASCADE, -- Глобальный уникальный идентификатор объекта
...
(
    ...
    HOUSETYPE  INTEGER REFERENCES as_house_types (ID) ON DELETE CASCADE,                       -- Основной тип дома
    OPERTYPEID SMALLINT    NOT NULL,                                                           -- Статус действия над записью – причина появления записи
    PREVID     BIGINT,                                                                         -- Идентификатор записи связывания с предыдущей исторической записью
    NEXTID     BIGINT,                                                                         -- Идентификатор записи связывания с последующей исторической записью
    UPDATEDATE DATE        NOT NULL,                                                           -- Дата внесения (обновления) записи
